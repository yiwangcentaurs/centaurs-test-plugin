...
var App = {
	...
	createPlugin: function () {
        ...
        /**
         * check api time usage
         * @param req - in next(req, res, err)
         * @param res - in next(req, res, err)
         * @param res - in next(req, res, err)
            ...
            request.post(
                ...
                function (err, response, body) {
                    ...
                    Debug.print({
                        func: 'timeUsage()',
                        type: 'msg',
                        msg: `sending ${info.name}() time usage to monitor server...`
                    });
                    if (!err && response.statusCode == 200) {
                        if (body.retcode === 0) {
                            // success
